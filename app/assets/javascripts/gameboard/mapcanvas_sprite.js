...
*	Draw hexagon shape
**/
MapCanvas.prototype.draw_shape = function(x, y, canvas, fill, stroke, player_colour_id) {

    if (player_colour_id == undefined) player_colour_id = 0;
    var img_tile = $("#tile"+player_colour_id)[0];
    canvas.drawImage(img_tile, x, y);
...
*	Draw border around hexagon shape
**/
MapCanvas.prototype.draw_border = function(x, y, canvas, border, img_tile_id) {
    for (var i=0; i<6; i++){
        // test binary bit, draw the border if the bit is 1
        if ((border >> i) & 1 == 1){
            var img_tile = $("#"+img_tile_id+i)[0];
            canvas.drawImage(img_tile, x, y);
        }
    }
};
...
**/
MapCanvas.prototype.draw_tile = function(row, col, player_colour_id, frontier_border, inside_border, canvas) {
	var i;
	var x, y;
    ...
    this.draw_shape(x, y, ctx, fill, fill, player_colour_id);		// draw shape
    ...
    this.draw_border(x, y, ctx, inside_border & parseInt("100011",2), "tile_border_in_lite");
    ...
    this.draw_border(x, y, ctx, inside_border & parseInt("011100",2), "tile_border_in_dark");
    ...
    this.draw_border(x, y, ctx, frontier_border, "tile_border");
    this.draw_border(x, y, ctx, frontier_border, "tile"+player_colour_id+"_border");
...
MapCanvas.prototype.draw_tiles = function (map_data, map_width, map_height, land_owner) {
    var temp_canvas = document.createElement('canvas');  // temporary canvas to pre-render all the tiles
    var map_canvas = document.getElementById("canvas_map");

    temp_canvas.width = map_canvas.width;
    temp_canvas.height = map_canvas.height;
	...
	for (r=0; r<map_height; r++){
		for (c=0; c<map_width; c++){
			...
			if (map_data[r*map_width + c] != 0){
				...
				this.draw_tile(r, c, land_owner[current_tile_id], frontier_border, inside_border, temp_canvas);
			}
		}
	}

    // draw temp canvas to actual canvas
